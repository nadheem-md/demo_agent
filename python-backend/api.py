            ...
            if state["context"].account_number:
                customer_info_response = CustomerInfoResponse(
                    customer=CustomerDetails(
                        ...
                        registration_id=str(state["context"].user_registration_id) if state["context"].user_registration_id else None,
                        company=state["context"].user_company_name,
                        location=state["context"].user_location,
                        conference_package=state["context"].user_conference_package,
                        membership_type=state["context"].user_registered_tracks,
                        primary_stream=state["context"].user_primary_stream,
                        secondary_stream=state["context"].user_secondary_stream,
                    ),
                    bookings=[]
                )
